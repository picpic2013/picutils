...
def getWarppingGrid(refCam: List[MyPerspectiveCamera],  # len(refCam) : B
    ...
    srcPosture = srcPosture.reshape(B * N, 4, 4) # (B * N) x 4 x 4
    srcK = srcK.reshape(B * N, 3, 3)             # (B * N) x 3 x 3
    ...
    oneTensor = torch.ones(B, 1, H * W, dtype=dtype, device=device)
    zeroTensor = torch.zeros(B, 1, H * W, dtype=dtype, device=device)
    tensor0001 = torch.tensor([0., 0., 0., 1.], dtype=dtype, device=device)
    normalize_base = torch.tensor([WS, HS], dtype=dtype, device=device) * 0.5

    # calculate world line
    refGrid = refCam[0].uv_grid.repeat(B, 1, 1, 1).view(B, 2, H * W)        # B x 2 x H x W => B x 2 x (H * W)
    ref_Dir_XYZ_Picture = torch.cat([refGrid, oneTensor.clone()], dim=1)    # B x 3 x (H * W)
    ref_Dir_XYZ_Camera = torch.bmm(refK_inv, ref_Dir_XYZ_Picture)           # B x 3 x (H * W)

    dstCameraAnchorPoint4 = torch.bmm(refPosture_inv, tensor0001.repeat(B, 1).unsqueeze(2)) # B x 4 x 1
    ...
    src_XYZ_camera = torch.bmm(srcPosture.repeat(NP, 1, 1), ref_XYZ_World)           # (B * N * NP) x 4 x (H * W)
    src_XYZ_camera = src_XYZ_camera[:, :3, :]                                        # (B * N * NP) x 3 x (H * W)
    src_XYZ_picture = torch.bmm(srcK.repeat(NP, 1, 1), src_XYZ_camera)               # (B * N * NP) x 3 x (H * W)
